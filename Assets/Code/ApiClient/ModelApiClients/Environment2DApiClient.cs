...
public class Environment2DApiClient : MonoBehaviour
{
    ...
    {
        webClient.SetToken(AuthManager.Instance.ApiToken);
        string route = "/environments";

        IWebRequestReponse webRequestResponse = await webClient.SendGetRequest(route);
        Debug.Log(ParseEnvironment2DListResponse(webRequestResponse));
    ...
    {
        Debug.Log($"create environment: {environment.name}");
        webClient.SetToken(AuthManager.Instance.ApiToken);
        string route = "/environments";
